...
class Reporte1PdfView(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        comando = 'select  reportes.id numreporte, reportes.nom_reporte reporte,reportes.cuerpo_sql, reportes.descripcion descripcion , reportes.encabezados encabezados, reportes.excel excel, reportes.pdf pdf,reportes.csv as csv,reportes.grilla, reportes.grafica  as grafica from dbo.Administracion_mae_reportes reportes where cast(reportes.id as char)  = cast(' + str(
            numreporte) + ' as char)'
        ...
        for numreporte,  reporte, cuerpo_sql, descripcion, encabezados, excel, pdf, csv , grilla, grafica   in cur.fetchall():
            reporteSeleccionado.append(
                {'numreporte': numreporte,  'reporte': reporte, 'cuerpo_sql': cuerpo_sql,
                 'descripcion': descripcion, 'encabezados':encabezados, 'excel':excel, 'pdf':pdf, 'csv':csv, 'grilla':grilla , 'grafica':grafica})
